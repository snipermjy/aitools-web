...
export default function NewTutorialPage() {
  ...
  const generateSlug = (title: string) => {
    ...
    const titleSlug = title
      ...
      .replace(/^-+|-+$/g, '')
      .substring(0, 30);
    return titleSlug ? `${titleSlug}-${timestamp}` : `tutorial-${timestamp}`;
  };
  ...
  const handleSubmit = async (e: React.FormEvent, status: 'draft' | 'published') => {
    ...
    try {
      ...
      alert(`✅ 教程已${status === 'published' ? '发布' : '保存为草稿'}！`);
    ...
    <AdminLayout>
      <div>
        <div className="flex items-center gap-4 mb-6">
          <Link href="/tutorials" className="text-text-secondary hover:text-primary transition-colors">
            <ArrowLeftIcon className="w-5 h-5" />
          </Link>
          <h1 className="text-2xl font-bold text-text-primary">发布AI教程</h1>
        </div>
        ...
        <form onSubmit={(e) => handleSubmit(e, 'draft')} className="space-y-6">
          ...
          <div className="card">
            ...
            <RichTextEditor
              ...
              placeholder="输入教程内容，支持 Markdown 格式..."
              minHeight="500px"
            />
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/tutorials" className="btn bg-gray-100 text-text-secondary hover:bg-gray-200">
              取消
            </Link>
          </div>
        </form>
